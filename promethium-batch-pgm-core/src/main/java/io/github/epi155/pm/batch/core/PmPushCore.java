package io.github.epi155.pm.batch.core;
...
import static io.github.epi155.pm.batch.fault.Fixed.JOB_NAME;
import static io.github.epi155.pm.batch.fault.Fixed.STEP_NAME;
...
abstract class PmPushCore<I> /*implements LoopSource<I>*/ {
        ...
        for (val status : statuses) {
            if (status.isDone()) {
                try {
                    ...
                    String place = BatchStepException.placeOf(cause.getStackTrace());
    ...
    public abstract class DoAsyncParallelFair<R> {
        ...
        protected DoAsyncParallelFair(int maxThread, Function<? super I, ? extends Future<? extends R>> asyncTransformer) {
            if (maxThread < 1)
...
//                    Thread.currentThread().interrupt();
    ...
    public abstract class DoParallelFair<R> {
        private final int maxThread;
        private final Function<? super I, ? extends R> transformer;
        private final Thread main;
        private final BlockingQueue<Future<? extends R>> queue;

        protected DoParallelFair(int maxThread, Function<? super I, ? extends R> transformer) {
            if (maxThread < 1)
                throw new IllegalArgumentException();
...
//                    Thread.currentThread().interrupt();
    ...
    public abstract class DoParallelWrite {
        private final int maxThread;
    ...
    public abstract class DoParallelRaw<R> {
        private final int maxThread;
        private final Function<? super I, ? extends R> transformer;
        private final Thread main;
        private final BlockingQueue<R> queue;

        protected DoParallelRaw(int maxThread, Function<? super I, ? extends R> transformer) {
            if (maxThread < 1)
                throw new IllegalArgumentException();
            this.maxThread = maxThread;
            this.transformer = transformer;
...
//                    Thread.currentThread().interrupt();
    ...
    public abstract class DoAsyncWrite {
        private final int maxThread;
        private final ExecutorService writerService;
