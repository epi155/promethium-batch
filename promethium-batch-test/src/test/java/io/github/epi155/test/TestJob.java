...
class TestJob {
    ...
    void job01() {
        ...
        int x = JCL.job("Job01")
                ...
                .join()
                .when(4, Cond.LE).execPgm(count4, this::step02) // execute if ok
                .when(4, Cond.GT, STEP03).execPgm(count5, this::step01) // execute in ko
                .complete();
        log.info("Job returnCode: {}", x);
    }
    ...
    void job02() {
        ...
        Proc<Param02> proc1 = Proc.create((p, s) -> s
                ...
                .cond(0, Cond.NE).execPgm(p, "Step02", this::step31)
        );

        Proc<Param02> proc01 = Proc.create((p, s) -> s
                .execPgm(p, count1, this::step21)
                .when(0, Cond.EQ).execPgm(p, count2, this::step21)
                .execProc("proc5", Proc.create(it -> it.execPgm("dummy", () -> 0)))
        );
        Proc<Param02> proc02 = Proc.create((p, s) -> s
                .execPgm(p, count1, this::step21)
                .push()
                .when(0, Cond.EQ).execProc(p, "proc1", proc01)
                .pop()
                .join()
        );
...
//                .parallel(2).forEachPgm(ls, MyCount::new, this::step05)
                .push()
                .when(0, Cond.EQ, "lock").execPgm(new MyCount("unlock"), this::step01)
                .exec(s -> s.returnCode("lock")
                        .filter(rcLock -> (rcLock == 0))
                        .ifPresent(rcLock -> s.execPgm(new MyCount("unlock"), this::step01)))
                .pop()
