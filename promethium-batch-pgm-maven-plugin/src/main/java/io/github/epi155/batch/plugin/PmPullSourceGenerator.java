...
public class PmPullSourceGenerator extends ClassSourceGeneratorLot {
    private static final String END_ARGS = ") {%n";
    private final int nmInp;
    private final GenerateContext gcx;

    public PmPullSourceGenerator(File baseDir, GenerateContext gcx, int nmInp) {
        super(baseDir, gcx);
        this.gcx = gcx;
        this.nmInp = nmInp;
    }
    ...
    protected void createClass(PrintModel ipw, Range range) throws FileNotFoundException {
        ...
        for(int k: range) {
            ...
            if (k==0) {
                ...
                new PullProcessGenerator(baseDir, gcx, nmInp).generate("PullProcess"+nmInp+"o" + k, k);
