...
public class AsyncLoopGenerator extends ClassSourceGeneratorOne {
    public AsyncLoopGenerator(File baseDir, GenerateContext gcx) {
        super(baseDir, gcx);
    }
    ...
    private void writeDeclareInterface(PrintModel ipw, int k) {
        ipw.printf("public interface AsyncLoop%d<I,", k);
        for (int i = 1; i <= k; i++) {
            ipw.putf("O%d", i);
            ipw.putf(i < k ? "," : "> {%n");
        }
